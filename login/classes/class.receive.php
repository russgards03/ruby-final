...
class Receive{
	...
	public function new_receive($desc){
		...
		$stmt = $this->conn->prepare("INSERT INTO tbl_receive(rec_description, rec_date_added, rec_time_added, rec_status) VALUES (?,?,?,?)");
		try {
	...
	public function save_to_inventory($id){
		...
		$stmt = $this->conn->prepare("INSERT INTO tbl_product_inv(rec_id, product_id, product_qty) VALUES (?,?,?)");
		try {
			$this->conn->beginTransaction();
			foreach ($data as $row){
				extract($row);
				$stmt->execute(array($rec_id,$product_id,$rec_qty));
			}
			//$id= $this->conn->lastInsertId();
			$this->conn->commit();
		}catch (Exception $e){
