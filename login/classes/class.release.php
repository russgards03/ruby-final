...
class Release{
	...
	public function new_release($desc){
		...
		$stmt = $this->conn->prepare("INSERT INTO tbl_release(rel_description, rel_date_added, rel_time_added, rel_status) VALUES (?,?,?,?)");
		try {
	...
	public function new_release_item($relid,$product_id,$qty){
		...
		$NOW = $NOW->format('Y-m-d H:i:s');
		$data = [
			[$relid,$product_id,$qty],
		];
		$stmt = $this->conn->prepare("INSERT INTO tbl_release_items(rel_id, product_id, rel_qty) VALUES (?,?,?)");
		try {
			$this->conn->beginTransaction();
			foreach ($data as $row)
			{
	...
	public function save_to_released($id){
		...
		$stmt = $this->conn->prepare("INSERT INTO tbl_release_inv(rel_id, product_id, product_qty) VALUES (?,?,?)");
		try {
			$this->conn->beginTransaction();
			foreach ($data as $row){
				extract($row);
